...
from keras.layers import Input, Dense, LSTM, Conv1D, Masking, Dropout, TimeDistributed, GlobalMaxPool1D
...
class Network(Model):
    ...
    def __init__(self, dim, batch_norm, dropout, rec_dropout, task,
                 ...
                 depth=1, sent_dim=40, embed_dim=768, conv_depth=1, **kwargs):
        ...
        X = Input(shape=(None, sent_dim, embed_dim), name='X')
        inputs = [X]

        # Time series convoluitons on sentence vectors
        for i in range(conv_depth):
            X = TimeDistributed(Conv1D(dim, 3, activation='relu'))(X)
